...
Questa sezione fornisce ulteriori dettagli sulle funzioni chiave e i vantaggi di ognuna delle quattro offerte di Direct Link. 
  ...
  * [**Host di IBM Cloud Direct Link dedicato**](#the-ibm-cloud-direct-link-dedicated-hosting-solution)

## La soluzione IBM Cloud Direct Link Exchange

La soluzione IBM Cloud Direct Link Exchange consente agli utenti di utilizzare un provider Cloud Exchange per fornire la connettività a {{site.data.keyword.BluSoftlayer_notm}}. Questa offerta normalmente fornisce la connettività a un costo ridotto, perché la connettività fisica da {{site.data.keyword.BluSoftlayer_notm}} al provider Cloud Exchange è già in atto, condivisa tra altri clienti.

**Casi di utilizzo comuni:** _Migliore per i carichi di lavoro ibridi e tra provider, per i trasferimenti di dati frequenti o grandi, per i carichi di lavoro privati e per la gestione dell'ambiente.  Questa opzione normalmente viene selezionata quando il PoP desiderato già dispone del provider IBM Cloud Direct Link Exchange desiderato._
 ...
 * **Tempo di distribuzione tipico:** per il provider Equinix il tempo di distribuzione tipico sarà in ore. Per gli altri provider 5-10 giorni dopo che il circuito raggiunge lo scambio e il tempo di distribuzione può essere di 30-60 giorni complessivi, a seconda dell'ubicazione e dei requisiti.

 * **Dettagli connessione trasversale:** le connessioni trasversali fisiche per l'interconnessione Cloud Exchange vengono conservate tra {{site.data.keyword.BluSoftlayer_notm}} e il provider Cloud Exchange. I clienti richiedono un "Circuito virtuale" dal provider Cloud Exchange, che stabilisce la connettività logica a {{site.data.keyword.BluSoftlayer_notm}}, una volta che sono interconnessi al provider Cloud Exchange.

 * **Opzioni di velocità della porta:** seleziona 50Mbps, 100Mbps, 200Mbps, 500Mbps o 1Gbps.

 * **Latenza approssimativa:** la latenza approssimativa è circa 1.5ms nell'area locale (data center con lo stesso prefisso di tre lettere, ad esempio DAL, AMS, MEL, ecc). Consulta http://lg.softlayer.com/ per le misurazioni della latenza PoP-to-PoP (P2P) dal vivo.
 ...
 * **Opzioni di instradamento Locale/Globale:** l'opzione di instradamento locale fornisce l'accesso ai data center con lo stesso prefisso di tre lettere come il PoP (DAL, AMS, MEL, ecc). L'opzione di instradamento globale è necessaria come un componente aggiuntivo per raggiungere i data center al di fuori di queste ubicazioni.
 
## La soluzione IBM Cloud Direct Link Connect

**Casi di utilizzo comuni** simili alla soluzione Direct Link Exchange. Offre più opzioni di velocità. Un buon punto di ingresso a costi più bassi.

![Figura 2](/images/Direct-Link-Connect.PNG)

* **Ubicazione di terminazione:** PoP (point of presence) {{site.data.keyword.BluSoftlayer_notm}}.

* **Tempo di distribuzione tipico:** 5-10 giorni dopo che il circuito raggiunge il scambio. Il tempo di distribuzione può essere di 30-60 giorni complessivi, a seconda dell'ubicazione e dei requisiti.

* **Dettagli connessione trasversale:** le connessioni trasversali fisiche per l'interconnessione Cloud Connect vengono conservate tra {{site.data.keyword.BluSoftlayer_notm}} e il provider Cloud Connect. I clienti richiedono un "Circuito virtuale" dal provider Cloud Connect, che stabilisce la connettività logica a {{site.data.keyword.BluSoftlayer_notm}}, una volta che sono interconnessi al provider Cloud Connect.
...
* **Ridondanza:** per stabilire la ridondanza per IBM Cloud Direct Link Connect, è necessaria la connettività in 2+ ubicazioni o la selezione di un'ubicazione con un XCR secondario disponibile che può essere utilizzata dal provider Cloud Connect.
 ...
 * **Tempo di distribuzione tipico:** 10-15 giorni lavorativi dopo che il nuovo circuito raggiunge il POP. Il tempo di distribuzione può essere di 30-60 giorni complessivi, a seconda dell'ubicazione e dei requisiti.
 ...
 * **Ridondanza:** {{site.data.keyword.BluSoftlayer_notm}} fornisce connessioni a due XCR (cross-connect router) come parte del prodotto. Per stabilire la connettività ridondante, i clienti configurano BGP con ECMP (equal-cost multipath).
