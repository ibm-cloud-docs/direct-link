 ...
 * **Local/Global Routing Options:** The Local Routing option is the default routing and  provides access to data centers with the same Market as the Direct Link PoP ( denoted for example  as DAL, AMS, MEL). The Global Routing option is required as an add-on to connect in-market IBM cloud resources to other IBM Cloud resources in data centers outside the local market. It is used to share workloads between IBM Cloud resources ( for example Dallas to Ashburn, or Dallas to Frankfurt).
